...
const postPatients = async (req, res) => {
  let { name, username, password, birth, phone, mail, province, city, number, street, document } =
    req.body;
  let idv4 = uuidv4();
  ...
  try {
    ...
    if ( !name || !username || !password || !birth || !phone || !mail || !province || !city || !number || !street || !document ) {
...
const putPatients = async (req, res) => {
  try {
    ...
    const { username, document, birth, phone, mail, province, city, number, street, } = req.body;
